...
const f7 = () => {
    let arr = [];  
    for (let i = 0; i < i7.value.length; i++) { // каждый символ добавляем в массив
        arr[i] = i7.value[i];
    }  
    let arrSet = new Set(arr);  // массив присваиваем в набор
    if (arr.length == arrSet.size && arr.length > 6) { // проверка условия и вывод 1 или 0
...
const f9 = our_set => { 
    let out = '';
    let arrSet9 = new Set(our_set);
    for ( let item of arrSet9) {
        out += item + ' ';
    }
    return out;
}
...
const f10 = (out_set, elem) => {
    let out = '';
    let arrSet10 = new Set(out_set);
    for( let item of arrSet10 ) {
        out += item + ' ';
    }
...
const f12 = () => {
    let newstr12 = str12.replace(/\s/g, '');
    let arr12 = newstr12.split('');
    let arr12Set = new Set(arr12);
    return arr12Set;
}
...
const f13 = () => {
    let newstr13 = str13.toLowerCase().replace(/\s/g, '');  // без пробелов и в одинаковом регистре
    let arr13 = newstr13.split('');
    let arr13Set = new Set(arr13);
    let out = '';
    let rez = '';
    let count = 1;
    
    for ( let char of arr13Set ) {
